...
import { ListForm, ListItem } from ".";
import { useEffect, useState } from "react";
import { DragDropContext, DropResult, Droppable } from "@hello-pangea/dnd";
import { useAction } from "@/src/hooks/useAction";
import { updateListOrder } from "@/src/actions/updateListOrder";
import { toast } from "sonner";
import { updateCardOrder } from "@/src/actions/updateCardOrder";
...
function reorder<T>(list: T[], startIndex: number, endIndex: number) {
  const result = Array.from(list); // copy list array
  const [removed] = result.splice(startIndex, 1); // remove 1 item from startIndex from list array and destruct the removed item
  result.splice(endIndex, 0, removed); // add the removed item to the endIndex
